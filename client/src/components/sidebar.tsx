...
export function Sidebar({ onQuestionClick }: SidebarProps) {
  return (
    <aside className="w-full lg:w-80 bg-card/90 backdrop-blur-sm border-r border-imperial-gold-200 p-6 lg:sticky lg:top-16 lg:h-screen lg:overflow-y-auto">
      {/* Welcome Section */}
      <div className="mb-8 p-6 bg-imperial-gold-100/60 border border-tang-red-200 rounded-xl phoenix-pattern section-card">
        ...
        <div className="space-y-3">
          {quickQuestions.map((item, index) => (
            <Button
              key={index}
              variant="ghost"
              className="w-full text-left p-3 bg-warm-cream-100/50 hover:bg-warm-cream-200 rounded-lg transition-all duration-200 suggestion-card h-auto justify-start border border-imperial-gold-200/50"
          ...
          {topics.map((topic, index) => (
            <Button
              ...
              className={`px-3 py-1 rounded-full text-xs font-medium cursor-pointer transition-all duration-200 h-auto border ${
                index % 3 === 0 
                  ? "bg-tang-red-100 text-tang-red-700 hover:bg-tang-red-200 border-tang-red-200" 
                  : index % 3 === 1
                  ? "bg-turquoise-blue-100 text-turquoise-blue-700 hover:bg-turquoise-blue-200 border-turquoise-blue-200"
                  : "bg-imperial-gold-100 text-imperial-gold-700 hover:bg-imperial-gold-200 border-imperial-gold-200"
              }`}
